<template>
    <div class="space-y-6 w-[88%] md:w-3/4 lg:w-1/2 mx-auto">
      <div class="flex justify-between items-center">
        <h1 class="md:text-xl font-bold">Saved Trips</h1>

        <div v-if="getAllSavedTripResponse != null" class="flex items-center">
          <label for="sort" class="text-[#BAAEA2] text-xs">Sort by:</label>
          <select v-model="tripListSort"  class="bg-[#FCF9F7] text-xs md:text-sm rounded-lg" name="sort" id="sort"  @change="getAllSavedTripListSort(tripListSort)">
      ...
      <div class="flex flex-col space-y-4" v-if="!getAllSavedTripResponse?.loading || getAllSavedTripResponse.length">
        <router-link :to="`/tripinfo/${trip.id}`" v-for="trip in getAllSavedTripResponse" :key="trip.id"
        ...
        >
          <div class="flex flex-col space-y-2 w-3/4">
            <span class="font-bold md:text-lg">{{ trip?.destination }}</span>
  ...
  <div class="flex flex-col space-y-6 w-full h-full justify-center items-center" v-if="getAllSavedTripResponse ===null">
    <img class="w-3/4 md:w-1/2" src="../static/empty.png" alt="">
    <p>No any saved trips.</p>
  ...
  const getAllSavedTripResponse = ref();
  const tripListSort = ref("desc");


  getAllSavedTripResponse.value = tripStore.getAllSavedTripResponse;
  watch(
    () => tripStore.getAllSavedTripResponse,
    (newValue) => {
        getAllSavedTripResponse.value = newValue;
    }
  );

  onMounted(() => {
    return tripStore
      .getAllSavedTripList("desc")
      .then((res) => {
  ...
  function getAllSavedTripListSort(sort){
    console.log("sorta bax",sort)
     tripStore
      .getAllSavedTripList(sort)
      .then((res) => {
        console.log("getAllTripList ugurlu result");
      })
      .catch((err) => {
